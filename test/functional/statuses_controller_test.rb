...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should be redirected when no user is logged in" do
    get :new
  ...
  test "should create status for the current user when logged in" do
    ...
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:sara).id }
    end
  ...
  test "should update status when no user is logged in" do
  ...
  test "should update status when current user is logged in" do
    sign_in users(:brad)
    put :update, id: @status, status: { content: @status.content, user_id: users(:sara).id }
    assert_response :redirect
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:brad).id
  end
